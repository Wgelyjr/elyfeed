...
# the git deploy key; root is fine and supported via RUNNER_ALLOW_RUNASROOT).
# Installs to /opt/elyfeed-runner and registers a systemd service
# (actions.runner.<org>-<repo>.<name>.service), so the runner survives
# reboots — replacing the old polling systemd service.
set -euo pipefail
...
# .env is written by config.sh on successful registration; its presence
# means this directory already holds a registered runner.
if [ -f .env ]; then
  echo "a runner is already registered here; remove $INSTALL_DIR first to reinstall" >&2
  exit 1
fi
